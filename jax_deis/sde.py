...
def get_sampler(sde, num_timesteps, eps_fn, order, highest_order=3, discr_method="quad", method="deis", last_step=False):
    ...
    rev_timesteps = jnp.asarray(np_rev_timesteps)
    ei_ab_coef = jnp.asarray(np_ei_ab_coef)
    ...
    def sampler(x0):
        ...
        eps_pred = jnp.asarray([x0,] * highest_order)
        img, _ = jax.lax.fori_loop(0, num_timesteps, ei_body_fn, (x0, eps_pred))
        return img
    return sampler